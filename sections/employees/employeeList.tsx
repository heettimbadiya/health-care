...
import { DeleteConfirmationModal } from '../../sections/employees/deleteConfirmationModal';
...
// Local storage key for employees (was in utils/storage.ts)
const EMPLOYEES_STORAGE_KEY = 'healthcare_employees';

// Storage helpers (previously in app/dashboard/employees/utils/storage.ts)
...
function createEmployee(employeeData: Omit<Employee, 'id' | 'createdAt' | 'updatedAt'>): Employee {
...
// Constants (previously in app/dashboard/employees/utils/constants.ts)
...
// Yup validation schema (replacing utils/validation.ts)
const employeeSchema = Yup.object({
    ...
    // password is only used in create mode here
    password: Yup.string()
...
async function validateEmployeeFormYup(formData: EmployeeFormData): Promise<{
    isValid: boolean;
...
// File to base64 (from utils/validation.ts)
...
export const EmployeeList = React.forwardRef<EmployeeListRef, EmployeeListProps>(
    ({ onRefresh }, ref) => {
        ...
        const handleCreateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
            ...
            const validation = await validateEmployeeFormYup(formData);
